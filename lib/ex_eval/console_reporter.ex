defmodule ExEval.ConsoleReporter do
      ...
      total_count = 
        ...
        |> Enum.reduce(0, fn module, acc ->
          if function_exported?(module, :__ex_eval_eval_cases__, 0) do
            acc + length(module.__ex_eval_eval_cases__())
          else
            acc
          end
        end)
      
      IO.puts("Running ExEval with seed: #{:rand.uniform(999999)}, max_cases: #{total_count}")
      
      if runner.options[:parallel] == false do
        IO.puts("")
      end
    else
      total_count = 
  ...
  defp print_trace_result(result) do
    ...
    test_description = if String.length(input_str) > 60 do
      String.slice(input_str, 0, 57) <> "..."
    else
      input_str
    end
    
    duration_str = if result[:duration_ms], do: " (#{format_duration(result.duration_ms)})", else: ""
  ...
  def print_summary(%ExEval.Runner{} = runner) do
    ...
    if failed > 0 || errors > 0 do
      IO.puts("\nRandomized with seed #{:rand.uniform(999999)}")
    end
  end
